...
const unscopables = {
 ...
 class Sandbox {
    fakeWindow: FakeWindow = createFakeWindow()
    proxy: Window
    updateKeys = new Set()
    listenerMap = new Map<string, EventListenerOrEventListenerObject[]>()
    timeoutList: number[] = []
    intervalList: number[] = []
    originKeys: string[] = []
    showlog = false
    ...
    constructor(showlog?: boolean) {
        ...
        // 定时器
        this.proxy.setInterval = (handler: CallableFunction, timeout?: number, ...args: any[]) => {
            if (!this.hasDisposed) {
        ...
        this.proxy.clearInterval = (intervalId: number) => {
        ...
        this.proxy.setTimeout = (handler: CallableFunction, timeout?: number, ...args: any[]) => {
            if (!this.hasDisposed) {
        ...
        this.proxy.clearTimeout = (timeoutId: number) => {
    ...
    dispose () {
        this.listenerMap.forEach((listeners, type) =>
            [...listeners].forEach((listener) => rawRemoveEventListener.call(window, type, listener))
        )
        ...
        this.timeoutList.forEach((timeoutId) =>
            rawWindowClearTimeout(timeoutId)
        )
        this.timeoutList = []
